...
class Keranjang extends BaseController
{
    ...
    public function checkout()
    {
        ...
        if ($id_produks && $jumlah_beli) {
            $cek_keranjang = $this->keranjangModel
                ...
                $data_view = [
                    'data_keranjang' => $cek_keranjang->getResult(),
                    'total' => 0
                ];

                foreach ($cek_keranjang->getResult() as $key => $value) {
                    $data_inputan_keranjang = [
                        'jumlah_beli' => $jumlah_beli[$key]
                    ];
    
                    $this->keranjangModel->update($value->id_keranjang, $data_inputan_keranjang);
                }
            }
        ...
        $cek_data_produk = $produkModel
        ->whereIn('id_produk', $id_produks)
        ->get();

        foreach ($cek_data_produk->getResult() as $key => $value) {
            
            $data_view['total'] += intval($value->harga) * $jumlah_beli[$key];
        }
        
        return view('keranjang/form_cekout', $data_view);
    }
    ...
    public function checkout_proses()
    {
        ...
        $id_keranjangs = $this->request->getPost('id_keranjangs');

        // mengambil data keranjang milik user
        $data_keranjang = $this->keranjangModel
            ->join('produk', 'produk.id_produk = keranjang.id_produk', 'LEFT')
            ->whereIn('id_keranjang',$id_keranjangs)
            ->where('id_user', $sess_user['id_user'])
            ->get();
